...
`parcel` takes as input a single entry asset, which could be any file type: JS, HTML, CSS, image(PNG/JPEG), etc. There are various asset types defined in `parcel` which know how to handle specific file types. The assets are parsed, their dependencies are extracted, and they are transformed to their final compiled form. This creates a tree of assets.
