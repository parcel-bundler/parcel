...
pub fn create_esm_loader(project_root: String) -> (ModuleResolver, ModuleLoader) {
  let module_resolver = ModuleResolver::new(project_root);
  let resolver = module_resolver.resolver.clone();
  (module_resolver, ModuleLoader { resolver })
}
  ...
  fn load<'js>(
      ...
      .resolve_module_type(Path::new(name), &Default::default())
    {
      Ok(ModuleType::Module) => {
        let source =
          std::fs::read_to_string(Path::new(name)).map_err(|e| rquickjs::Error::Loading {
            name: name.into(),
