...
use parcel_resolver::{Cache, OsFileSystem, PathId, Resolution, Resolver, SpecifierType};
...
fn main() {
  ...
  let cjs_resolver = Resolver::node(PathId::new(&cwd), &cache);
  ...
  deps.keys().for_each(|dep| {
    ...
    if let Resolution::Path(p) = resolved {
      match build_esm_graph(&p.to_path_buf(), &cwd, &cache, &esm_graph_cache, &fs) {
        Ok(_res) => {
          // #[cfg(debug_assertions)]
          // println!("{:?}", res)
        }
