...
#[napi]
pub fn tree_shake(opts: JsObject, env: Env) -> napi::Result<JsUnknown> {
  let config: parcel_js_swc_core::TreeShakeOptions = env.from_js_value(opts)?;
  let result = parcel_js_swc_core::tree_shake(config).ok();
  env.to_js_value(&result)
}
...
mod native_only {
  ...
  #[napi]
  pub fn tree_shake_async(opts: JsObject, env: Env) -> napi::Result<JsObject> {
    let config: parcel_js_swc_core::TreeShakeOptions = env.from_js_value(opts)?;
    let (deferred, promise) = env.create_deferred()?;

    rayon::spawn(move || {
      let res = parcel_js_swc_core::tree_shake(config).ok();
      deferred.resolve(move |env| env.to_js_value(&res))
    });
