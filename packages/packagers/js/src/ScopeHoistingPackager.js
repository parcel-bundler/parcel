...
export class ScopeHoistingPackager {
  ...
  /** wrapped root asset -> assets in the island of the root (excluding root) */
  ...
  async loadAssets(): Promise<void> {
    ...
    while (wrapped.size > 0) {
      ...
      for (let root of [...wrapped]) {
        ...
        this.bundle.traverseAssets((a, ctx, actions) => {
          ...
          // console.log('aRoot', root, a, aRoot, usedByMultiple.has(a));
          if (aRoot != null) {
            if (aRoot !== root) {
              ...
              this.bundle.traverseAssets((b, _, actions) => {
                ...
                usedByMultiple.add(b);
              }, a);
              actions.skipChildren();
              return;
            } else {
              // already part of this island
              return;
            }
          }

          // Don't add root to its own island
          if (a === root) {
            return;
          }
          ...
          if (this.usedByUnwrappedEntries.has(a) || usedByMultiple.has(a)) {
          ...
          usedByMultiple.delete(a);
          this.unwrappedAssets.delete(a);
          // console.log('add', root, a);
          island.add(a);
    ...
    //   invariant(
    //     this.unwrappedAssets.has(asset) ||
  ...
  buildAsset(
    ...
    if (this.shouldSkipAsset(asset) && !this.islands.has(asset)) {
      let depCode = '';
      let lineCount = 0;
      for (let dep of deps) {
...
`;
  ...
  getSymbolResolution(
    ...
    let island = new Set([
      ...(this.islands.get(this.islandRoot.get(parentAsset) ?? parentAsset) ??
        this.unwrappedAssets),
      parentAsset,
    ]);
    let {
      asset: resolvedAsset,
      exportSymbol,
      symbol,
    } = this.bundleGraph.getSymbolResolution(
      resolved,
      imported,
      this.bundle,
      island,
    ...
    if (
      ...
      (!this.bundle.hasAsset(resolvedAsset) ||
        this.islands.has(resolvedAsset) ||
