...
function getNeededPluginNames(targets: BabelTargets): Array<string> {
  return presetEnv(
    {assertVersion: () => true},
    {targets: targets}
...
function shouldCompileFurther(
  ...
  let sourcePlugins = new Set(getNeededPluginNames(sourceBabelTargets));
  let appPlugins = getNeededPluginNames(appBabelTargets);
