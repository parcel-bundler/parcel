...
export default class EfficientGraph<TEdgeType: number = 1> {
  ...
  resizeEdges(size: number) {
    ...
    // console.log('getAllEdges', this.getAllEdges());
    ...
    for (let i = 0; i < this.nodes.length; i += NODE_SIZE) {
      ...
      for (
        ...
        if (lastIndex != null) {
          ...
          //   lastIndex,
          //   index,
          //   correctIndex: this.indexOf(18, 19, 1),
          // });
          // for (let i = 0; i < this.edges.length; i += EDGE_SIZE) {
          //   if (this.edges[i + FROM] == 18 && this.edges[i + TO] == 19) {
    ...
    for (let i = 0; i < this.nodes.length; i += NODE_SIZE) {
      ...
      while (nextEdge) {
        ...
        edgeObjs.push({
          ...
          index: edgeIndex,
          nextIn: this.edges[edgeIndex + NEXT_IN],
          nextOut: this.edges[edgeIndex + NEXT_OUT],
        });
