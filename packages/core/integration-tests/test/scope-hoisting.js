...
const configSpecifiers = [
  '@parcel/config-default',
  path.resolve('experimental-bundler-config.json'),
];

for (let config of configSpecifiers) {
  ...
  describe('scope hoisting with ' + config, function() {
    ...
    describe('es6', function() {
      ...
      itSkipExperimental(
        'supports live bindings across bundles',
        async function() {
          let b = await bundle(
            ['a.html', 'b.html'].map(f =>
              path.join(
      ...
      describe('tree shaking dynamic imports', function() {
        it.skip('supports tree shaking statically analyzable dynamic import: destructued await assignment', async function() {
          ...
          assert(
            b.isDependencySkipped(findDependency(b, 'async.js', './a1.js')),
          );
          ...
          assert.deepStrictEqual(
            new Set(
              b.getUsedSymbols(
                findDependency(
                  b,
                  'await-declaration-namespace.js',
                  './async.js',
                ),
              ),
            ),
        ...
        it('supports tree shaking statically analyzable dynamic import: destructured then', async function() {
          ...
          assert.deepStrictEqual(
            new Set(
              b.getUsedSymbols(findDependency(b, 'then.js', './async.js')),
            ),
            new Set(['foo', 'thing']),
          );
          assert(
      ...
      it('should support the jsx pragma', async function() {
        let b = await bundle(
          path.join(
            __dirname,
            '/integration/scope-hoisting/es6/jsx-pragma/a.js',
          ),
        );
      ...
      describe('correctly updates used symbols on changes', () => {
        itSkipExperimental('dependency symbols change', async function() {
        ...
        itSkipExperimental('add and remove dependency', async function() {
          let testDir = path.join(
            __dirname,
          ...
          try {
            ...
            let assetC = nullthrows(
              findAsset(bundleEvent.bundleGraph, 'd1.js'),
            );
            assert.deepStrictEqual(
            ...
            let assetD = nullthrows(
              findAsset(bundleEvent.bundleGraph, 'd2.js'),
            );
            assert.deepStrictEqual(
        ...
        itSkipExperimental(
          'add and remove dependency with namespace',
          async function() {
            let testDir = path.join(
              __dirname,
      ...
      describe('sideEffects: false', function() {
        itSkipExperimental(
          'supports excluding unused CSS imports',
          async function() {
            let b = await bundle(
              path.join(
                __dirname,
        ...
        itSkipExperimental(
          'supports deferring an unused ES6 re-exports (namespace used)',
          async function() {
            let b = await bundle(
              path.join(
                __dirname,
        ...
        itSkipExperimental(
          'supports deferring an unused ES6 re-exports (reexport named used)',
          async function() {
            let b = await bundle(
              path.join(
                __dirname,
        ...
        itSkipExperimental(
          'supports deferring an unused ES6 re-exports (namespace rename used)',
          async function() {
            let b = await bundle(
              path.join(
                __dirname,
        ...
        itSkipExperimental(
          'supports deferring an unused ES6 re-exports (direct export used)',
          async function() {
            let b = await bundle(
              path.join(
                __dirname,
        ...
        itSkipExperimental('supports chained ES6 re-exports', async function() {
          let b = await bundle(
            path.join(
              __dirname,
        ...
        itSkipExperimental(
          'supports excluding CommonJS (CommonJS used)',
          async function() {
            let b = await bundle(
              path.join(
                __dirname,
            ...
            assert.deepStrictEqual(
              new Set(
                b.getUsedSymbols(nullthrows(findAsset(b, 'commonjs.js'))),
              ),
              // the exports object is used freely
              new Set(['*', 'message2']),
            );
      ...
      it('can reexport urls to raw assets', async () => {
        ...
        assert.deepStrictEqual(
          new Set(
            b.getUsedSymbols(
              findDependency(b, 'reexports.js', 'url:./foo.png'),
            ),
          ),
          new Set(['default']),
        );
    ...
    itSkipExperimental(
      'should include prelude in shared worker bundles',
      async function() {
        let b = await bundle(
          path.join(__dirname, '/integration/worker-shared/index.js'),
          {
    ...
    itSkipExperimental(
      'loads another bundle from a dynamic import with a shared dependency only when necessary',
      async () => {
        let b = await bundle(
