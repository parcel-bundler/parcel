...
async function diffAssets(
  ...
  let i = 0;
  for (let file of codes1) {
    fs.writeFileSync('fixed-bundle-' + i, file);
    i += 1;
  }
  for (let file of codes2) {
    fs.writeFileSync('broken-bundle-' + i, file);
    i += 1;
  }
  ...
  for (let bundle1 of bundles1) {
    ...
    bundle1.traverseAssets((asset1: Asset) => {
      let asset2 = null;
      try {
        asset2 = graph2.getAssetById(asset1.id);
      } catch (err) {
    ...
    for (let {asset1, asset2} of assetPairs) {
      ...
      /// THIS IS THE PROBLEM
      // GRAPH1 - (WORKING) returns "barrel-inner" / "undefined"
      // GRAPH2 - (BROKEN) returns "empty" / "undefined"
      if (asset1.filePath.includes('barrel-inner.js')) {
        const resolution1 = graph1.getSymbolResolution(
          asset1,
          'thing',
          bundle1,
        );
        const resolution2 = graph2.getSymbolResolution(
          asset2,
          'thing',
          bundle2,
        );
        assert.deepStrictEqual(
          resolution1.asset.filePath,
          resolution2.asset.filePath,
        );
        assert.deepStrictEqual(resolution1, resolution2);
      }
      if (asset1.filePath.includes('barrel-outer.js')) {
        const resolution1 = graph1.getSymbolResolution(
          asset1,
          'thing',
          bundle1,
      ...
      // if (asset1.filePath.includes('empty.js')) {
      //   const resolution1 = graph1.getUsedSymbols(asset1);
      //   const resolution2 = graph2.getUsedSymbols(asset2);
      //   assert.deepStrictEqual(Array.from(resolution1), Array.from(resolution2));
      // }

      // if (asset1.filePath.includes('empty.js')) {
      //   const resolution1 = graph1.getSymbolResolution(asset1, '*', bundle1);
      //   const resolution2 = graph2.getSymbolResolution(asset2, '*', bundle2);
      //   assert.deepStrictEqual(resolution1.asset.filePath, resolution2.asset.filePath);
      //   assert.deepStrictEqual(resolution1, resolution2);
      // }
    }
    ...
    await fsFixture(overlayFS, __dirname)`
      empty-re-export
        ...
        barrel-outer.js:
          // a.js
          export * from './thing.js';
          export * from './barrel-inner.js';
        index.js:
          // index.js
          // import {thing} from './barrel-outer.js';
          // output(thing);

          output(import('./barrel-outer.js').then(({thing}) => { return thing; }));
        index.html:
          <script src="./index.js" type="module" />
        yarn.lock:
          // Required for config loading
    ...
    await fsFixture(overlayFS, __dirname)`
      empty-re-export
        ...
        index.js:
          ...
          output(import('./barrel-outer.js').then(({thing, other}) => { console.log(other); return thing; }));
        index.html:
          <script src="./index.js" type="module" />
        yarn.lock:
          // Required for config loading
