...
for (let config of configSpecifiers) {
  ...
  describe('html with ' + config, function() {
    beforeEach(async () => {
    ...
    it('should support meta tags', async function() {
      ...
      assert(
        html.includes(`<meta name="msapplication-config" content="none">`),
      );
    ...
    it('should process inline non-js scripts', async function() {
      let b = await bundle(
        path.join(
          __dirname,
          '/integration/html-inline-coffeescript/index.html',
        ),
        {
          defaultTargetOptions: {
            shouldOptimize: true,
    ...
    it('should normalize case of SVG elements and attributes when minified', async function() {
      ...
      assert(
        output.includes('<feGaussianBlur in="SourceGraphic" stdDeviation='),
      );
    });
  });
}
