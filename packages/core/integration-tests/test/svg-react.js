...
describe('svg-react', function () {
  ...
  it('should support transforming SVGs to react components with jsx pipeline', async function () {
    let b = await bundle(
      path.join(__dirname, '/integration/svg-react-with-jsx-pipeline/react.js'),
      {
        defaultConfig: path.join(
          __dirname,
          'integration/custom-configs/.parcelrc-svg-react-with-jsx-pipeline',
        ),
      },
    );
