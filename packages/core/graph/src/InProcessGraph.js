...
import {toNodeId, fromNodeId, ALL_EDGE_TYPES} from './types';
import assert from 'assert';
import nullthrows from 'nullthrows';

export type InProcessGraphOpts<TNode, TEdgeType: number = 1> = {|
...
export type SerializedInProcessGraph<TNode, TEdgeType: number = 1> = {|
  nodes: Map<NodeId, TNode>,
  edges: AdjacencyListMap<TEdgeType | NullEdgeType>,
  rootNodeId: ?NodeId,
  nextNodeId: number,
...
export default class InProcessGraph<TNode, TEdgeType: number = 1> {
  nodes: Map<NodeId, TNode>;
  ...
  static deserialize(
    opts: InProcessGraphOpts<TNode, TEdgeType>,
  ): InProcessGraph<TNode, TEdgeType> {
    return new this({
      nodes: opts.nodes,
      edges: opts.edges,
  ...
  serialize(): SerializedInProcessGraph<TNode, TEdgeType | NullEdgeType> {
    return {
      nodes: this.nodes,
  ...
  getNodeIdsConnectedTo(
    ...
    if (type === ALL_EDGE_TYPES) {
      ...
      nodes = new Set(inboundByType.get((type: any))?.values() ?? []);
    }
  ...
  getNodeIdsConnectedFrom(
    ...
    if (type === ALL_EDGE_TYPES) {
      ...
      nodes = new Set(outboundByType.get((type: any))?.values() ?? []);
    }
