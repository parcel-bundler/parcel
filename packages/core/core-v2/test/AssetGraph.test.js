...
describe('AssetGraph', () => {
  ...
  it('updateFile should update the asset and dep nodes a file is connected to', () => {
    ...
    let assets = [
      { filePath, hash: '#1', dependencies: [{ sourcePath, moduleSpecifier: './utils' }] },
      { filePath, hash: '#2', dependencies: [{ sourcePath, moduleSpecifier: './styles' }] },
    ...
    assert(!graph.incompleteNodes.has(nodeFromFile(file).id));
    assert(graph.incompleteNodes.has(nodeFromDep({ sourcePath, moduleSpecifier: './utils' }).id));
    assert(graph.incompleteNodes.has(nodeFromDep({ sourcePath, moduleSpecifier: './styles' }).id));
